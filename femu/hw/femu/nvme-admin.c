...
static const uint32_t nvme_cse_iocs_nvm[256] = {
    ...
    [NVME_CMD_COMPUTE_PARITY]       = NVME_CMD_EFF_CSUPP,
    [NVME_CMD_REMAP]                = NVME_CMD_EFF_CSUPP,
};
...
{
    femu_debug("NVME_ADMIN_OPCODE: 0x%x\n", cmd->opcode);
    switch (cmd->opcode) {
    ...
    case NVME_ADM_CMD_ASYNC_EV_REQ:
        femu_debug("admin cmd, async_event_req\n");
        return NVME_SUCCESS;
...
void nvme_process_sq_admin(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq))) {
        ...
        if(cmd.opcode == NVME_ADM_CMD_ASYNC_EV_REQ) continue;
