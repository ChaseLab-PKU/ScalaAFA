...
static void nvme_post_cqe(NvmeCQueue *cq, NvmeRequest *req)
{
    ...
    if(req->cmd.opcode == NVME_CMD_WRITE || req->cmd.opcode == NVME_CMD_READ || req->cmd.opcode == NVME_CMD_COMPUTE_PARITY)
        cqe->n.rsvd = cpu_to_le32(req->gc_flag);
...
uint16_t nvme_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    // req->transient = false;
    req->transient = le64_to_cpu(rw->rsvd2);
    // if(req->transient) femu_err("transient\n");
    req->user_chunk_no = (uint32_t)le64_to_cpu(rw->mptr);
    req->slot = (uint8_t)(le64_to_cpu(rw->mptr) >> 32);
    if(req->transient) {
        femu_debug("transient data %ld %ld %d %d\n", rw->mptr, le64_to_cpu(rw->mptr), req->user_chunk_no, req->slot);
    }
...
static uint16_t nvme_io_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    if (nsid == 0 || nsid > n->num_namespaces) {
        femu_err("%s, opcode: 0x%x, fuse: 0x%x, res1: 0x%x, psdt: 0x%x, cid: 0x%x, nsid: 0x%x,\
                res2: 0x%lx, mptr: 0x%lx, cdw10: 0x%x, cdw11: 0x%x, cdw12: 0x%x, cdw13: 0x%x, cdw14: 0x%x,\
                cdw15: 0x%x\n", __func__, cmd->opcode, cmd->fuse, cmd->res1, cmd->psdt, cmd->cid, cmd->nsid, cmd->res2, cmd->mptr,
    ...
    femu_debug("NVME_IO_OPCODE: 0x%x\n", cmd->opcode);
    switch (cmd->opcode) {
    case NVME_CMD_FLUSH:
