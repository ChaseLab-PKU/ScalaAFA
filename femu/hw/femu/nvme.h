...
typedef struct NvmeRequest {
    ...
    uint64_t                dlba;               // used for computing parity
    uint64_t                num_chunks;         // used for computing parity
    NvmeCmd                 cmd;
    NvmeCqe                 cqe;
    uint8_t                 cmd_opcode;
    bool                    gc_flag;            // used for gc detection
    bool                    transient;          // mark the data which is transient     
...
typedef struct FemuExtCtrlOps {
    ...
    uint16_t (*admin_cmd)(struct FemuCtrl *, NvmeCmd *, NvmeCqe *cqe);
