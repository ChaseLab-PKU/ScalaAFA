...
static void
_nvme_ns_cmd_setup_request_afa_spec(struct spdk_nvme_ns *ns, struct nvme_request *req,
			   uint32_t opc, uint64_t lba, uint32_t lba_count,
			   uint32_t io_flags, uint16_t apptag_mask, uint16_t apptag, uint8_t transient, uint32_t user_chunk_no, uint8_t slot)
{
	struct spdk_nvme_cmd	*cmd;
	...
	*(uint64_t *)&cmd->rsvd2 = transient;
	cmd->mptr = ((uint64_t)slot << 32 | (uint64_t) user_chunk_no);
}
...
static inline struct nvme_request *
_nvme_ns_cmd_rw_afa_spec(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
		const struct nvme_payload *payload, uint32_t payload_offset, uint32_t md_offset,
		uint64_t lba, uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t opc,
		uint32_t io_flags, uint16_t apptag_mask, uint16_t apptag, bool check_sgl, int *rc, uint8_t transient, uint32_t user_chunk_no, uint8_t slot)
{
	struct nvme_request	*req;
	uint32_t		sector_size = _nvme_get_host_buffer_sector_size(ns, io_flags);
	uint32_t		sectors_per_max_io = _nvme_get_sectors_per_max_io(ns, io_flags);
	...
	_nvme_ns_cmd_setup_request_afa_spec(ns, req, opc, lba, lba_count, io_flags, apptag_mask, apptag, transient, user_chunk_no, slot);
	return req;
}
...
int spdk_nvme_ns_cmd_write_afa_spec(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				   void *buffer, void *metadata,
				   uint64_t lba, uint32_t lba_count, spdk_nvme_cmd_cb cb_fn,
				   void *cb_arg, uint32_t io_flags,
				   uint16_t apptag_mask, uint16_t apptag, uint8_t transient, uint32_t user_chunk_no, uint8_t slot)
{
	struct nvme_request *req;
	struct nvme_payload payload;
	int rc = 0;
	...
	req = _nvme_ns_cmd_rw_afa_spec(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_WRITE,
			      io_flags, apptag_mask, apptag, false, &rc, transient, user_chunk_no, slot);
	if (req != NULL) {
		return nvme_qpair_submit_request(qpair, req);
	} else {
		return nvme_ns_map_failure_rc(lba_count,
...
int
spdk_nvme_ns_cmd_writev_afa_spec(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				uint64_t lba, uint32_t lba_count,
				spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags,
				spdk_nvme_req_reset_sgl_cb reset_sgl_fn,
				spdk_nvme_req_next_sge_cb next_sge_fn, void *metadata,
				uint16_t apptag_mask, uint16_t apptag, uint8_t transient, uint32_t user_chunk_no, uint8_t slot)
{
	...
	req = _nvme_ns_cmd_rw_afa_spec(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_WRITE,
			      io_flags, apptag_mask, apptag, true, &rc, transient, user_chunk_no, slot);
	if (req != NULL) {
		return nvme_qpair_submit_request(qpair, req);
	} else {
		return nvme_ns_map_failure_rc(lba_count,
...
int 
spdk_nvme_ns_cmd_compute_parity(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				  uint64_t src_lba, uint8_t num_data_chunks,
				  uint64_t dst_lba, uint32_t lba_count,
				  spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags)
{
	struct nvme_request	*req;
	struct spdk_nvme_cmd	*cmd;
	...
	cmd = &req->cmd;
	cmd->opc = SPDK_NVME_OPC_COMPUTE_PARITY;
	cmd->nsid = ns->id;

	tmp_lba = (uint64_t *)&cmd->cdw10;
	*tmp_lba = dst_lba;
	cmd->cdw12 = lba_count;
	tmp_lba = (uint64_t *)&cmd->cdw13;
	*tmp_lba = src_lba;
	cmd->cdw15 = num_data_chunks;

	return nvme_qpair_submit_request(qpair, req);
}

int 
spdk_nvme_ns_cmd_remap(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				  uint64_t src_lba, uint64_t dst_lba, uint32_t lba_count,
				  spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags)
{
	struct nvme_request	*req;
	struct spdk_nvme_cmd	*cmd;
