...
static int bdev_writev_blocks_with_md(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
				      ...
				      struct spdk_bdev_ext_io_opts *opts, bool copy_opts);
static int bdev_writev_blocks_afa_spec(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
				      struct iovec *iov, int iovcnt, void *md_buf,
				      uint64_t offset_blocks, uint64_t num_blocks,
				      spdk_bdev_io_completion_cb cb, void *cb_arg,
				      struct spdk_bdev_ext_io_opts *opts, bool copy_opts,
					  bool transient, uint32_t user_chunk_no, uint8_t slot);
	     ...
	     spdk_bdev_io_completion_cb cb)
{
	...
	bdev_io->u.bdev.afa.gc_flag = 2;
	bdev_io->u.bdev.afa.transient_hint = 0;
}
					  ...
					  bool transient, uint32_t user_chunk_no, uint8_t slot)
{
	...
	bdev_io->internal.ext_opts = opts;
	bdev_io->u.bdev.ext_opts = opts;
	bdev_io->u.bdev.afa.transient_hint = transient;
	bdev_io->u.bdev.afa.user_chunk_no = user_chunk_no;
	bdev_io->u.bdev.afa.slot = slot;
...
int spdk_bdev_writev_blocks_afa_spec(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
			    struct iovec *iov, int iovcnt,
			    uint64_t offset_blocks, uint64_t num_blocks,
			    spdk_bdev_io_completion_cb cb, void *cb_arg, bool transient, uint32_t user_chunk_no, uint8_t slot)
{
	return bdev_writev_blocks_afa_spec(desc, ch, iov, iovcnt, NULL, offset_blocks,
					  num_blocks, cb, cb_arg, NULL, false, transient, user_chunk_no, slot);
}
...
int 
spdk_bdev_compute_parity(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch, 
				  uint64_t src_offset_blocks, uint8_t num_data_chunks, 
			      uint64_t dst_offset_blocks, uint64_t num_blocks,
			      spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	struct spdk_bdev *bdev = spdk_bdev_desc_get_bdev(desc);
	struct spdk_bdev_io *bdev_io;
	...
	bdev_io->internal.ch = channel;
	bdev_io->internal.desc = desc;
	bdev_io->type = SPDK_BDEV_IO_TYPE_COMPUTE_PARITY;
	bdev_io->u.bdev.num_blocks = num_blocks;
	bdev_io->u.bdev.offset_blocks = dst_offset_blocks;
	bdev_io->u.bdev.src.num_data_chunks = num_data_chunks;
	bdev_io->u.bdev.src.src_offset_blocks = src_offset_blocks;
	bdev_io_init(bdev_io, bdev, cb_arg, cb);

	_bdev_io_submit_ext(desc, bdev_io, NULL, false);

	return 0;
}

int
spdk_bdev_remap_blocks(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch, 
				  uint64_t src_offset_blocks, uint64_t dst_offset_blocks, uint64_t num_blocks,
			      spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	struct spdk_bdev *bdev = spdk_bdev_desc_get_bdev(desc);
	struct spdk_bdev_io *bdev_io;
	...
	if (!bdev_io_valid_blocks(bdev, dst_offset_blocks, num_blocks) || !bdev_io_valid_blocks(bdev, src_offset_blocks, num_blocks)) {
		return -EINVAL;
	}
	...
	bdev_io->internal.ch = channel;
	bdev_io->internal.desc = desc;
	bdev_io->type = SPDK_BDEV_IO_TYPE_REMAP;
	bdev_io->u.bdev.num_blocks = num_blocks;
	bdev_io->u.bdev.offset_blocks = dst_offset_blocks;
	bdev_io->u.bdev.src.src_offset_blocks = src_offset_blocks;
	bdev_io_init(bdev_io, bdev, cb_arg, cb);
