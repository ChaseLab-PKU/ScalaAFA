...
struct spdk_bdev_io {
	...
	union {
		struct {
			...
			struct {
				/* 0 -> not in gc, 1 -> in gc, 2 -> unknown */ 
				uint8_t gc_flag;			
				/* 0 -> transient data, 1 -> otherwise */
				uint8_t transient_hint;		
				/* slot in afa layer (user to recovery afa2dev map) */
				uint8_t slot;
				/* user chunk number of this write request (used to recovery user2afa map) */
