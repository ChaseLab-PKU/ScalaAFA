...
/**
 * Submit a compute parity I/O to the specified NVMe namespace.
 *
 * The command is submitted to a qpair allocated by spdk_nvme_ctrlr_alloc_io_qpair().
 * The user must ensure that only one thread submits I/O on a given qpair at any
 * given time.
 ...
 * \param src_lba Starting LBA of data chunks.
 * \param num_data_chunks Number of data chunks to be computed.
 * \param dst_lba Dst LBA to write computing result.
 * \param lba_count Length (in sectors) of data chunks.
...
int spdk_nvme_ns_cmd_compute_parity(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				  uint64_t src_lba, uint8_t num_data_chunks,
				  uint64_t dst_lba, uint32_t lba_count,
				  spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags);

/**
 * Submit a remap I/O to the specified NVMe namespace.
 *
 * The command is submitted to a qpair allocated by spdk_nvme_ctrlr_alloc_io_qpair().
 * The user must ensure that only one thread submits I/O on a given qpair at any
 * given time.
 ...
 * \param ns NVMe namespace to submit the write zeroes I/O.
 * \param qpair I/O queue pair to submit the request.
 * \param src_lba Src LBA to be remap.
 * \param dst_lba Dst LBA remapped to.
 * \param lba_count Length (in sectors) of remap area.
 * \param cb_fn Callback function to invoke when the I/O is completed.
 * \param cb_arg Argument to pass to the callback function.
 * \param io_flags Set flags, defined by the SPDK_NVME_IO_FLAGS_* entries in
 *
...
int spdk_nvme_ns_cmd_remap(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
 ...
 * \param apptag_mask Application tag mask.
 * \param apptag Application tag to use end-to-end protection information.
 * \param transient whether the data is transient
 ...
 * -ENXIO: The qpair is failed at the transport level.
 */
int spdk_nvme_ns_cmd_writev_afa_spec(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				    uint64_t lba, uint32_t lba_count,
				    spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags,
				    spdk_nvme_req_reset_sgl_cb reset_sgl_fn,
				    spdk_nvme_req_next_sge_cb next_sge_fn, void *metadata,
