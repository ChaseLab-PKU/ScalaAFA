...
enum spdk_bdev_io_type {
	...
	SPDK_BDEV_IO_TYPE_REMAP,
	SPDK_BDEV_IO_TYPE_COMPUTE_PARITY,
 ...
 * \param cb Called when the request is complete.
 * \param cb_arg Argument passed to cb.
 * \param transient the data to be written is transient
 * \param user_chunk_no user chunk number of this write request (used to recovery user2afa map)
 * \param slot slot in afa layer (user to recovery afa2dev map)
 *
 * \return 0 on success. On success, the callback will always
 * be called (even if the request ultimately failed). Return
 * negated errno on failure, in which case the callback will not be called.
...
/**
 * Submit a compute parity request to the bdev on the given channel. 
 *
 * \ingroup bdev_io_submit_functions
 *
 * \param desc Block device descriptor.
 * \param ch I/O channel. Obtained by calling spdk_bdev_get_io_channel().
 * \param src_offset_blocks The offset of data chunks to be computed.
 * \param num_data_chunks Numver of data chunks.
 * \param dst_offset_chunks The offset to write computing result.
 * \param num_blocks The size of data chunk.
 * \param cb Called when the request is complete.
 * \param cb_arg Argument passed to cb.
 *
 * \return 0 on success. On success, the callback will always
 ...
 *   * -EBADF - desc not open for writing
 */
int spdk_bdev_compute_parity(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch, 
				  uint64_t src_offset_blocks, uint8_t num_data_chunks, 
			      uint64_t dst_offset_blocks, uint64_t num_blocks,
			      spdk_bdev_io_completion_cb cb, void *cb_arg);

/**
 * Submit a remap request to the bdev on the given channel. 
 *
 * \ingroup bdev_io_submit_functions
 *
 * \param desc Block device descriptor.
 * \param ch I/O channel. Obtained by calling spdk_bdev_get_io_channel().
 * \param src_offset_blocks The offset of src address.
 * \param dst_offset_chunks The offset of dst address.
 * \param num_blocks The size of remap area.
 * \param cb Called when the request is complete.
 * \param cb_arg Argument passed to cb.
 *
 * \return 0 on success. On success, the callback will always
 ...
 *   * -EBADF - desc not open for writing
 */
int spdk_bdev_remap_blocks(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch, 
