...
static int bdev_nvme_compute_parity(struct nvme_bdev_io *bio, uint8_t num_data_chunks, uint64_t src_lba,
			    uint64_t lba_count, uint64_t dst_lba);
static int bdev_nvme_remap(struct nvme_bdev_io *bio, uint64_t src_lba, uint64_t dst_lba, uint64_t lba_count);
...
complete:
	bio->retry_count = 0;
	if(bdev_io->type == SPDK_BDEV_IO_TYPE_READ || bdev_io->type == SPDK_BDEV_IO_TYPE_WRITE || bdev_io->type == SPDK_BDEV_IO_TYPE_COMPUTE_PARITY)
		bdev_io->u.bdev.afa.gc_flag = cpl->cdw1;
...
bdev_nvme_submit_request(struct spdk_io_channel *ch, struct spdk_bdev_io *bdev_io)
{
	...
	case SPDK_BDEV_IO_TYPE_ABORT:
		...
		bdev_nvme_abort(nbdev_ch,
				...
				nbdev_io_to_abort);
		break;
	case SPDK_BDEV_IO_TYPE_COMPUTE_PARITY:
		rc = bdev_nvme_compute_parity(nbdev_io,
				      bdev_io->u.bdev.src.num_data_chunks,
					  bdev_io->u.bdev.src.src_offset_blocks,
				      bdev_io->u.bdev.num_blocks,
				      bdev_io->u.bdev.offset_blocks);
		break;
	case SPDK_BDEV_IO_TYPE_REMAP:
		rc = bdev_nvme_remap(nbdev_io,
					  bdev_io->u.bdev.src.src_offset_blocks,
		 ...
		 uint32_t flags, struct spdk_bdev_ext_io_opts *ext_opts)
{
	...
	if (ext_opts) {
		...
		rc = spdk_nvme_ns_cmd_write_afa_spec(ns, qpair, iov[0].iov_base, md, lba,
						    lba_count,
						    bdev_nvme_writev_done, bio,
						    flags,
						    0, 0, bdev_io->u.bdev.afa.transient_hint, bdev_io->u.bdev.afa.user_chunk_no, bdev_io->u.bdev.afa.slot);
		
	} else {
		rc = spdk_nvme_ns_cmd_writev_afa_spec(ns, qpair, lba, lba_count,
						     bdev_nvme_writev_done, bio, flags,
						     bdev_nvme_queued_reset_sgl, bdev_nvme_queued_next_sge,
						     md, 0, 0, bdev_io->u.bdev.afa.transient_hint, bdev_io->u.bdev.afa.user_chunk_no, bdev_io->u.bdev.afa.slot);
	}
...
static void
bdev_nvme_compute_parity_done(void *ref, const struct spdk_nvme_cpl *cpl)
{
	struct nvme_bdev_io *bio = ref;
...
static int
bdev_nvme_compute_parity(struct nvme_bdev_io *bio, uint8_t num_data_chunks, uint64_t src_lba,
			    uint64_t lba_count, uint64_t dst_lba)
{
	struct spdk_nvme_ns *ns = bio->io_path->nvme_ns->ns;
	struct spdk_nvme_qpair *qpair = bio->io_path->qpair->qpair;
	...
	SPDK_DEBUGLOG(bdev_nvme, "compute parity src: %#" PRIx64 ", dst : %#" PRIx64 ", num_data_chunks : %"PRIx8", chunk_size : %"PRId64"  \n",
		      src_lba, dst_lba, num_data_chunks, lba_count);
	
	rc = spdk_nvme_ns_cmd_compute_parity(ns, qpair, src_lba, num_data_chunks, dst_lba, lba_count, 
						     bdev_nvme_compute_parity_done, bio, 0);

	if (rc != 0 && rc != -ENOMEM) {
		SPDK_ERRLOG("compute parity failed: rc = %d\n", rc);
	}
	return rc;
}

static void
bdev_nvme_remap_done(void *ref, const struct spdk_nvme_cpl *cpl)
{
	struct nvme_bdev_io *bio = ref;
...
static int 
bdev_nvme_remap(struct nvme_bdev_io *bio, uint64_t src_lba, uint64_t dst_lba, uint64_t lba_count)
{
	struct spdk_nvme_ns *ns = bio->io_path->nvme_ns->ns;
	struct spdk_nvme_qpair *qpair = bio->io_path->qpair->qpair;
	int rc;

	SPDK_DEBUGLOG(bdev_nvme, "remap src: %#" PRIx64 ", dst : %#" PRIx64 ", size : %"PRId64"  \n",
		      src_lba, dst_lba, lba_count);
	
	rc = spdk_nvme_ns_cmd_remap(ns, qpair, src_lba, dst_lba, lba_count, 
						     bdev_nvme_remap_done, bio, 0);
