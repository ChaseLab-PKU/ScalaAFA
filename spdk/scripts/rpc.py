...
Format: 'user:u1 secret:s1 muser:mu1 msecret:ms1,user:u2 secret:s2 muser:mu2 msecret:ms2'""", required=False)
    ...
    *** Mutually exclusive with --disable-chap ***""", action='store_true')
    ...
    # afa
    def bdev_afa_list_all(args):
        print_array(rpc.bdev.bdev_afa_list_all(args.client))

    p = subparsers.add_parser('bdev_afa_list_all',
                              help="""This is used to list the names of all afa bdevs.""")
    p.set_defaults(func=bdev_afa_list_all)

    def bdev_afa_list_base_bdevs(args):
        print_array(rpc.bdev.bdev_afa_list_base_bdevs(args.client,
                                                 name=args.name))

    p = subparsers.add_parser('bdev_afa_list_base_bdevs',
                              help="""list all existing afa bdevs""")
    p.add_argument('name', help='name of the afa bdev')
    p.set_defaults(func=bdev_afa_list_base_bdevs)

    def bdev_afa_create(args):
        base_bdevs = []
        for u in args.base_bdevs.strip().split(" "):
            base_bdevs.append(u)

        rpc.bdev.bdev_afa_create(args.client,
                                 name=args.name,
                                 chunk_size_kb=args.chunk_size_kb,
                                 num_data_base_bdevs = args.num_data_base_bdevs,
                                 num_parity_base_bdevs = args.num_parity_base_bdevs,
                                 num_spare_base_bdevs = args.num_spare_base_bdevs,
                                 base_bdevs=base_bdevs)
    p = subparsers.add_parser('bdev_afa_create', help='Create new afa bdev')
    p.add_argument('-n', '--name', help='afa bdev name', required=True)
    p.add_argument('-c', '--chunk_size_kb', help='chunk size of afa in KB', type=int, required=True)
    p.add_argument('-k', '--num_data_base_bdevs', help='number of data base bdevs', type=int, required=True)
    p.add_argument('-m', '--num_parity_base_bdevs', help='rnumber of parity base bdevs', type=int, required=True)
    p.add_argument('-s', '--num_spare_base_bdevs', help='number of spare base bdevs', type=int, required=True)
    p.add_argument('-b', '--base_bdevs', help='base bdevs name, whitespace separated list in quotes', required=True)
    p.set_defaults(func=bdev_afa_create)

    def bdev_afa_delete(args):
        rpc.bdev.bdev_afa_delete(args.client,
                                  name=args.name)
    p = subparsers.add_parser('bdev_afa_delete', help='Delete existing afa bdev')
    p.add_argument('name', help='afa bdev name')
    p.set_defaults(func=bdev_afa_delete)
